 ...
 *  @author Kevin Wayne
 */
public class TrieMini implements Iterable<String> {
           // extended ASCII
    private static final int R = 26;

    private Node root;      // root of trie
    private int N;          // number of keys in trie
    private char startFromLetter;
    ...
    private Node get(Node x, String key, int d) {
        ...
        char c =(char)(key.charAt(d) - startFromLetter);
        return get(x.next[c], key, d+1);
    }
    public Node getChild(Node x, char cKey)
    {
        if (x == null) return null;
        char c =(char)(cKey - startFromLetter);
        return x.next[c];
    }
    public Node getRoot()
    {
    ...
    private void collect(Node x, StringBuilder prefix, Queue<String> results) {
        ...
        for (char c = 0; c < R; c++) {
            prefix.append((char)(c+startFromLetter));
